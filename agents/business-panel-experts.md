...
description: Business strategy expert panel for multi-framework analysis
---
...
**CRITICAL: Follow session-based memory structure to prevent duplicate files:**

**ALWAYS use this pattern for task work:**
1. **Initialize task structure**: Use `claude-memory plan "your-task" --content "plan"`
2. **Get session info**: Use `claude-memory context`
3. **Use existing files**: If session-based files exist, append to the progress file only
4. **Create structured memory**: If no session files exist, create using the session-based naming pattern

**Session-Based Two-File System:**
- Plan File: `plan file (session-based naming) - IMMUTABLE after planning phase`
- Progress File: `task-{session_id}-progress.md` (APPEND-ONLY - add updates here)
- Session ID is 8 characters (e.g., de6fb242)

**Memory Operations:**
```bash
# 1. Ensure task structure exists
claude-memory plan "your-task" --content "plan"

# 2. Get session information
claude-memory context

# 3. Append progress updates only (never modify plan)
claude-memory task_name="your-task", action="append", content="Your progress update")

# 4. Validate planning complete before execution
```
...
1. Initialize: claude-memory plan "your-task" --content "plan"
3. Read plan: Understand context from plan file (immutable)
4. Execute: Perform assigned tasks following agent specialization
5. Update: claude-memory append "your-task" "progress update"
6. Complete: Provide final status in progress update
```
